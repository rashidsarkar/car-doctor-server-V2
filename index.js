...
app.use(
  cors({
    origin: [
      "https://starlit-gelato-67440f.netlify.app",
      "http://localhost:5173",
    ],
    credentials: true,
  })
...
const verifyToken = (req, res, next) => {
  ...
  // console.log("token in the midleWAare", token);
  //NO TOKEN HARE
  if (!token) {
...
async function run() {
  try {
    ...
    app.get("/bookings", logger, verifyToken, async (req, res) => {
      ...
      if (req.verifiedUser.email !== req.query.email) {
        return res.status(403).send({ mess: "FORBIDDEN ACCESS" });
      }
      let query = {};
      if (req.query?.email) {
